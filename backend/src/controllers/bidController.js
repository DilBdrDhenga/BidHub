import { asyncHandler } from "../utils/asyncHandler.js";
import ApiResponse from "../utils/ApiResponse.js";
import Bid from "../models/bidModel.js";
import Auction from "../models/auctionModel.js";

// @desc Add bid on item
// @route POST /api/v1/bids/:itemId
// @access Private
const addBidOnItem = asyncHandler(async (req, res) => {
    // res.send("test");
    
    try {
        // extracting amount from the requested body
        const { amount } = req.body;
    
        // validate amount
        if (!amount) {
            return res.status(400).json(new ApiResponse(400, "Amount is required."));
        }
        console.log("User provided amount is: ", amount);
    
        // finding the auction item
        let item = await Auction.findById(req.params.id); 
        // checking if item exist or not
        if (!item) {
            return res.status(404).json(new ApiResponse(404, "Item not found."));
        }
        console.log("Fetched item .... ", item);
    
        
        // Ensure item.bids is initialized
        // if (!item.bids) {
        //     item.bids = []; // Initialize as an empty array if undefined
        // }
        
        // Log bids before pushing
        // console.log("Bids before adding new bid: ", item.bids);
        
        // Check if the new bid is higher than the current highest bid
        // if (amount <= item.currentHighestBid) {
        //     return res.status(400).json(new ApiResponse(400, "Bid must be higher than the current highest bid."));
        // }

        // creating a new bid
        const newBid = new Bid ({
            bidder: req.user._id,
            auction: req.params.id,
            bidAmount: req.body.amount,
        });
        // saving the new bid
        await newBid.save();
        
        // updating item with new bid
        item.bids.push(newBid._id);
        item.startingPrice = amount;    // updates the current highest bid
        // saving the updated item
        await item.save();
    
        return res
            .status(201)
            .json(new ApiResponse(201, "Bid placed successfully.", newBid));
        } catch (error) {
        return res
            .status(500)
            .json(new ApiResponse(500, error?.message || "Internal server error."));
        }
});




// @desc Get all bids by user ID 
// @route GET /api/bids/get-all-bids-item
// @access Private
const getBidsByUser = asyncHandler(async (req, res) => {
    // res.send("test");
    try {
        // fetching bids
        const bids = await Bid.find({ bidder: req.user._id })
            .populate("auction", "title description startingPrice")
            .sort({ createdAt: -1 })
  
        console.log(bids, "bids....");
        
        // checking for bids 
        if (!bids) {
            return res.status(404).json(new ApiResponse(404, "No bids found"));
        }
        // if (bids.length === 0) {
        //     return res.status(404).json(new ApiResponse(404, "No bids found."));
        // }
        
        return res.status(200).json(new ApiResponse(200, "All bids by user.", { bids: bids }));
        
        } catch (error) {
        return res.status(500).json(new ApiResponse(500, error?.message || "Internal server error."));
    }
});
  
  


// @desc Get all bids by Auction ID
// @route GET /api/bids/get-all-bids/:auctionId
// @access Public
const getAllBidsByAuctionId = asyncHandler(async (req, res) => {
    // res.send("test");
    
    try {
        // fetching bids
        const bids = await Bid
            .find({ auction: req.params.auctionId })
            .populate("bidder", "fullName profilePicture")
            .sort({ createdAt: -1 })

        // checking if bids exist or not   
        if (!bids) {
            return res.status(404).json(new ApiResponse(404, "No bids found."));
        }
        // if (bids.length === 0) {
        //     return res.status(404).json(new ApiResponse(404, "No bids found."));
        // }

        return res.status(200).json(new ApiResponse(200, "All bids by auction", {bids: bids}));
    } catch (error) {
        return res.status(500).json(new ApiResponse(500, error?.message || "Internal server error."));
    }
});
  


//@desc Get all a winner of an auction
//@route GET /api/auctions/:id/winner
//@access Public

const getWinnerOfAuction = asyncHandler(async (req, res) => {
    console.log(req.params.id, "req.params.id");
    const bids = await Bid.find({ auction: req.params.id });
    console.log(bids, "auction.........");
  
    if (!bids) {
      return res.status(404).json(new ApiResponse(404, "Auction not found"));
    }
    if (bids.length === 0) {
      return res.status(406).json(new ApiResponse(406, "No bids found"));
    }
  
    // getting the max bid from the array
    // getting the user id of the max bid
    // getting the user from the user id



    let maxBidId = bids[0]._id;
    let maxAmount = bids[0].bidAmount;
    for (let i = 1; i < bids.length; i++) {
      if (bids[i].bidAmount > maxAmount) {
        maxAmount = bids[i].bidAmount;
        maxBidId = bids[i]._id;
      }
    }
  
    console.log(maxAmount, "maxAmount");
    console.log(maxBidId, "maxBid");
    const auction = await Auction.findById(req.params.id);

    // setting the winner of the auction to the user
    const winnerUser = await Bid.findById(maxBidId).populate(
      "bidder",
      "fullName email phone profilePicture"
    );
  
    console.log(winnerUser, "winnerUser");
    auction.winner = maxBidId;

    
    // setting the auction to over
    auction.status = "over";
    
    // saving the auction
    await auction.save();
  
    return res
      .status(200)
      .json(new ApiResponse(200, "Winner of the auction", winnerUser));
  });
  



export {
    addBidOnItem,
    getBidsByUser,
    getAllBidsByAuctionId,
    getWinnerOfAuction,
};